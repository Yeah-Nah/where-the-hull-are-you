...
class cameraFeedProcessor:
    ...
    def camera_feed_connect(self):
        ...
        camRgb = self.pipeline.create(dai.node.Camera).build(dai.CameraBoardSocket.CAM_B)

        # Setup neural network input/output
        cam_out = camRgb.requestOutput((CAM_WIDTH, CAM_HEIGHT), dai.ImgFrame.Type.BGR888p, fps=30)
        video_out = camRgb.requestOutput((CAM_WIDTH, CAM_HEIGHT), dai.ImgFrame.Type.BGR888p, fps=30)
